...
function Awake()
{
	...
	print(GameObject.Find("Player_STBH_2").GetComponent(Control_windowsstandalone).Order.length );
//GameObject.Find("Player").GetComponent(Control_windowsstandalone).cam = GameObject.Find("Player").GetComponent(Control_windowsstandalone).Order[kovetkezo_cam];
...
if (Input.GetKeyDown("d")) {cam_switch = 2;}
...
ControlObject.GetComponent(Control_windowsstandalone).vege_signal = 1;
Imsorry.enabled = true;
	...
	if ( kovetkezo_cam == GameObject.Find("Player_STBH").GetComponent(Control_windowsstandalone).Order.length ) {Application.LoadLevel ("scenechanger");}
	else { print(kovetkezo_cam);GameObject.Find("Player_STBH").GetComponent(Control_windowsstandalone).cam = GameObject.Find("Player_STBH").GetComponent(Control_windowsstandalone).Order[kovetkezo_cam]; }
	...
	GameObject.Find("Player_STBH_2").GetComponent(Control_windowsstandalone).taskid = task_changed;
...
if (col.gameObject == Alien)
	...
	Takeme.Play();
	YellowBag.GetComponent.<Renderer>().enabled= false;
	BlueBag.GetComponent.<Renderer>().enabled= false;
	if (szine == 2)
	{//bonus = "                                    Yellow";
	LilAlienYellow.enabled = true;
	YellowSpaceShip.GetComponent.<Collider>().enabled= true;
	task_changed = task_changed +1;}
	else if (szine == 1)
	{//bonus = "                                    Blue";
	LilAlienBlue.enabled = true;
	BlueSpaceShip.GetComponent.<Collider>().enabled= true;
	task_changed = task_changed +1;}
...
{	bonus = " ";
	StartCoroutine("Helyvaltas");
	BlueSpaceShip.GetComponent.<Collider>().enabled= false;
	YellowSpaceShip.GetComponent.<Collider>().enabled= false;
	if (cam_switch == 2){
	if (kovetkezo_cam == 5) {kovetkezo_cam = 0;};
	GameObject.Find("Player_STBH").GetComponent(Control_windowsstandalone).cam = GameObject.Find("Player_STBH").GetComponent(Control_windowsstandalone).Order[kovetkezo_cam];kovetkezo_cam++;
	}
	if (ind == 28)
	{
	//ControlObject.GetComponent(Control_windowsstandalone).vege_signal = 1;
...
{	bonus = " ";
	...
	while (Order_d.Count != 0) {
		...
		GameObject.Find("Player_STBH").GetComponent(Control_windowsstandalone).Order = temp_d;
		print(GameObject.Find("Player_STBH").GetComponent(Control_windowsstandalone).Order);
	};
	GameObject.Find("Player_STBH").GetComponent(Control_windowsstandalone).cam = GameObject.Find("Player_STBH").GetComponent(Control_windowsstandalone).Order[kovetkezo_cam];kovetkezo_cam++;
	}
	if (ind == 28)
...
function Helyvaltas ()
{
	...
	if (szine == 2)
		{YellowBag.GetComponent.<Renderer>().enabled= true;
		BlueBag.GetComponent.<Renderer>().enabled= false;
		task_changed = 3;}
		else if (szine == 1)
		{BlueBag.GetComponent.<Renderer>().enabled= true;
		YellowBag.GetComponent.<Renderer>().enabled= false;
		task_changed = 1;}
